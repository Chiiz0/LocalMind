...
export function createProjectResourceTools(
  ...
  const receipt = async (resourceId: string) => {
    ...
    return {
      ...
      url: `/intelligence?project=${encodeURIComponent(projectId)}&resource=${encodeURIComponent(resourceId)}`,
    };
  };
    ...
    project_publication_prepare: defineTool({
      ...
      execute: async ({ resource_id, kind }, execute) => {
        ...
        return {
          ...
          url: `/intelligence?project=${encodeURIComponent(projectId)}&resource=${encodeURIComponent(resource_id)}&publication=${encodeURIComponent(publication.id)}`,
        };
      },
    }),
                    ...
                    const saved =
                      await models.copilotProjectAgentRuntime.execute(
                        ...
                        async () => {
                          execute.signal?.throwIfAborted();
                          if (!tool.execute)
                            throw new BadRequest('Project tool is unavailable');
...
const ProjectResourceCommandSchema = z
  .object({
    version: z.literal(1),
    toolName: z.enum([
      'doc_create',
