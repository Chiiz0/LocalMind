...
export class CopilotDocumentOperationModel extends BaseModel {
  @Transactional()
  async legacyProjectOperations(input: ProjectActor & { after?: string }) {
    await this.models.projectResource.assertMember(input, true);
    if (input.after && input.after.length > 256)
      throw new BadRequest('Invalid legacy operation cursor');
    const rows = await this.db.copilotDocumentOperation.findMany({
      where: {
    ...
    for (const row of rows) {
      if (row.projectMigrationStatus) continue;
      const status = row.createdDocumentAt
        ? 'external_result'
        : row.status === 'cancelled'
          ? 'cancelled'
          : 'pending';
      const updated = await this.db.copilotDocumentOperation.updateMany({
        where: { id: row.id, projectMigrationStatus: null },
        data: { projectMigrationStatus: status, projectMigrationRevision: 1 },
      });
      if (updated.count)
        await this.legacyEvent({
          ...row,
          projectMigrationStatus: status,
          projectMigrationRevision: 1,
        });
      row.projectMigrationStatus = status;
      row.projectMigrationRevision = 1;
    }
    return rows;
  }

  async legacyProjectOperation(input: ProjectActor & { operationId: string }) {
    await this.models.projectResource.assertMember(input);
    const row = await this.db.copilotDocumentOperation.findFirst({
      where: {
    ...
    if (!row) throw new NotFound('Legacy Project operation is unavailable');
    return row;
  }

  @Transactional<TransactionalAdapterPrisma>({ timeout: 60000 })
  async recoverLegacyProjectOperation(
    input: ProjectActor & {
      operationId: string;
      expectedRevision: number;
      action: 'recover' | 'cancel';
    },
    copy: (operation: CopilotDocumentOperation) => Promise<{ id: string }>
  ) {
    await this.models.projectResource.assertMember(input, true);
    await this.db
      .$queryRaw`SELECT id FROM copilot_document_operations WHERE id = ${input.operationId} FOR UPDATE`;
    const row = await this.legacyProjectOperation(input);
    if (
      row.projectMigrationStatus === 'complete' ||
      row.projectMigrationStatus === 'external_result'
    )
      return row;
    if (
      !row.projectMigrationStatus ||
      row.projectMigrationRevision !== input.expectedRevision
    )
      throw new BadRequest(
        'Legacy Project request changed; reload before recovery'
      );
    if (input.action === 'cancel' && row.projectMigrationStatus === 'cancelled')
      return row;
    const resource = input.action === 'recover' ? await copy(row) : null;
    if (resource)
      await this.models.projectResource.get({
        ...input,
        resourceId: resource.id,
      });
    const next = await this.db.copilotDocumentOperation.update({
      where: { id: row.id },
      data: {
        projectMigrationStatus: resource ? 'complete' : 'cancelled',
        projectMigrationRevision: { increment: 1 },
        projectMigrationResourceId: resource?.id,
      },
    });
    await this.legacyEvent(next);
    return next;
  }

  @Transactional()
  async blockLegacyProjectOperation(
    input: ProjectActor & { operationId: string; expectedRevision: number }
  ) {
    const row = await this.legacyProjectOperation(input);
    const changed = await this.db.copilotDocumentOperation.updateMany({
      where: {
        id: row.id,
        projectMigrationRevision: input.expectedRevision,
        projectMigrationStatus: { in: ['pending', 'blocked', 'cancelled'] },
      },
      data: {
    ...
    if (changed.count)
      await this.legacyEvent({
        ...row,
        projectMigrationStatus: 'blocked',
        projectMigrationRevision: row.projectMigrationRevision + 1,
      });
  }

  private legacyEvent(row: CopilotDocumentOperation) {
    return this.db.copilotDocumentOperationEvent.create({
      data: {
        operationId: row.id,
        eventType: 'project_migration',
        detail: {
          revision: row.projectMigrationRevision,
          status: row.projectMigrationStatus,
  ...
  async legacyProjectConversations(input: ProjectActor & { after?: string }) {
    await this.models.projectResource.assertMember(input);
  ...
  async legacyProjectMessages(
    input: ProjectActor & { sessionId: string; after?: string }
  ) {
    await this.models.projectResource.assertMember(input);
    ...
    if (!session)
      throw new NotFound('Historical Project conversation is unavailable');
    if (
      input.after &&
      !(await this.db.aiSessionMessage.findFirst({
        where: { id: input.after, sessionId: input.sessionId },
      }))
    )
